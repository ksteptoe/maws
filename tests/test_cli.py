...
from maws.cli import cli


def test_ec2_terminate_requires_confirm(monkeypatch):
    # Make describe_instances return something deterministic
    monkeypatch.setattr(
        api,
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["ec2", "terminate", "i-1"])
...
def test_ec2_terminate_dry_run_calls_api(monkeypatch):
    ...
    monkeypatch.setattr(api, "ec2_set_delete_on_termination", fake_dot)
    monkeypatch.setattr(api, "ec2_terminate_instances", fake_term)
